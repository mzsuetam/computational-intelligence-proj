...
# %%
w = [1.0, 1.0, 1.0, 1.0, 2.0, 1.2, 1.0]
print(df_classes.assign(weight=w)[['class_id', 'name', 'weight']].to_markdown())
loss_weights = torch.tensor(w).to(dls.device) # slightly favor small classes
...
learn_cse.load('best_segmentation_combined_loss', weights_only=True)


loss_func = FocalLoss(weight=loss_weights)

learn_cse.loss_func = loss_func

suggested_lr = learn_cse.lr_find()

accept_lr = input("Accept suggested learning rate? (y/n): ")
if accept_lr.lower() != 'y':
    suggested_lr = None
...
model_codename_name = 'cse_phase_4_3_focal_loss' 

learn_cse.fit_one_cycle(
    100, 
    lr_max=slice(lr/100, lr),
    cbs=[
        SaveCB(model_codename_name),
        LogCB(f'{model_codename_name}_log.csv'),
